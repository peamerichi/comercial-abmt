        ...
        if (this._configTab === 'empresa') {
            ...
            <div class="card"><div class="card-header"><span class="card-title">${LI("coins",20)} Impostos e Margens</span></div>
                <div class="form-row-3">
                    ...
                    <div class="form-group"><label>Desconto à vista (%)</label><input type="number" step="0.1" class="form-control" id="cfg-desconto-avista" value="${config.desconto_avista_percentual || 5}"></div>
                </div>
                <p style="font-size:11px;color:var(--text-secondary);margin-bottom:12px">A taxa de juros calcula o custo financeiro das vendas a prazo. O desconto à vista é a referência de desconto que o vendedor pode oferecer no pagamento à vista. <strong>Uso interno — não aparece no PDF do cliente.</strong></p>
    ...
    async salvarConfigImpostos(btn) {
        await this._configSave(btn, async () => {
            ...
            const res = await this.api('/api/config', { method: 'PUT', body: {
                ...
                desconto_avista_percentual: document.getElementById('cfg-desconto-avista').value,
